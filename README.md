...
# 필요한 패키지 설치
pip install seaborn
pip install pandas numpy torch matplotlib scikit-learn openpyxl
...
향후 Colab등 클라우드 기반 서비스를 이용하여 각 자치구 별 분석도 가능할 것으로 보임. 이번 프로젝트에서는 진행하지 못했음.
...
자치구 간 연관관계 분석 및 클러스터링 가능성 탐색하면 더 발전된 모델로 변모 가능 예상됩니다.
